...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_atob", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_atob": ["*.pyi", "py.typed"],
        "tree_sitter_atob.queries": ["*.scm"],
    },
    ext_package="tree_sitter_atob",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_atob/binding.c",
                "src/parser.c",
            ],
            define_macros=[
